...
Herb and spice inventory with smart recipe recommendations. Client-rendered SPA backed by Supabase.
...
-- append-only: meal_plan_entries is upserted per weekday and keeps no history,
-- so Auto Plan reads this to avoid recent repeats / spot never-tried recipes.
...
| `recipe_plan_history` | `user_id = auth.uid()` | insert only, `user_id = auth.uid()` (append-only: no update/delete policy) |
...
`features/plan/auto-plan.ts` — a pure, synchronous, unit-tested function, same
shape as `rank.ts`.

```ts
type CollectionScope = { system: boolean; mine: boolean; saved: boolean }  // OR'd together

type AutoPlanInput = {
  recipes: Array<{ id: string; name: string; diet_tags: string[]; ingredients: string[]
                    source: 'system' | 'user'; isSaved: boolean; area: string | null }>
  pantry: string[]
  restrictions: string[]
  disliked: string[]
  history: Record<string, string>   // recipe_id -> most recent planned_date (YYYY-MM-DD)
  scope: CollectionScope
  cuisines: string[]                 // restrict to recipes whose `area` is in this list; empty = no restriction
  newOnly: boolean                  // restrict to never-planned recipes, when enough qualify
  minCount: number                  // days to fill; newOnly relaxes below this
  today: string                     // injected for deterministic tests
}

export function buildAutoPlanCandidates(input: AutoPlanInput): {
  candidates: Array<{ id: string; name: string; area: string | null; coverage: number
                       missingCount: number; daysSincePlanned: number }>  // Infinity if never planned
  relaxedNewOnly: boolean
}

export function pickRandom<T>(pool: T[], count: number, rng?: () => number): T[]
```

Rules:

1. Keep a recipe if it's in the requested `scope` — system, the user's own
   (`source = 'user'`), and/or saved (`saved_recipes`) — independently OR'd, so
   a mixed scope (e.g. "mine + saved, no system") is a normal case, not a
   special one.
2. Same diet/dislike filtering as `rank.ts`, and the same pantry-coverage math.
3. If `cuisines` is non-empty, keep only recipes whose `area` is in it (exact
   match against the `areas` reference table — no free-text normalization
   needed, unlike pantry/dislikes).
4. "Avoid repeats" is a **soft ranking signal**, not a hard filter: sort by
   `daysSincePlanned` desc (never-planned first), then coverage desc, then name
   asc. This is deliberate — a hard 14-day exclusion needs a fallback tier for
   when a small recipe library runs dry; ranking by recency never runs dry, it
   just starts resurfacing the least-recently-cooked recipe.
5. The "haven't tried before" toggle (`newOnly`) restricts to
   `daysSincePlanned === Infinity`, but only when that leaves at least
   `minCount` candidates (evaluated after scope/diet/cuisine filtering) —
   otherwise it's ignored for that generation (`relaxedNewOnly: true`) rather
   than leaving days unfilled.

History source: `recipe_plan_history` (see Schema) — an append-only log, since
`meal_plan_entries` is upserted per weekday and can't answer "when was this
last cooked?" on its own. `api.upsertDay` inserts one row every time a recipe
is assigned to a day slot (manual pick or Auto Plan's "Apply"), so history
accrues automatically without a separate logging call.

Randomization is client-side and explicitly not part of the pure function:
`useAutoPlanCandidates()` composes pantry + profile + recipes-with-ingredients
+ saved recipes + history into one scored, sorted pool; `AutoPlanPanel`
fetches it once and calls `pickRandom()` to fill the active days. "Re-roll"
calls `pickRandom()` again on the same already-fetched pool — no refetch —
and "Apply" commits the preview through the normal `useUpsertPlanDay`
mutation.

`useAutoPlanCandidates()` also runs the algorithm a second time internally
with `cuisines: []` and `newOnly: false` to get the scope+diet+dislike-filtered
pool on its own, and derives `availableCuisines` (distinct `area` values) from
it — so the cuisine chips `AutoPlanPanel` offers are always ones that will
actually return results, independent of whatever cuisine/newOnly the user has
currently selected.

`AutoPlanPanel` is a 4-step guided flow (pantry shortcut → pick days → pick
source/cuisine → generate & review), replacing `WeekGrid` on `/plan` while
open; `PlanRoute` owns the open/closed toggle and shows `WeekGrid` otherwise.
...
['plan-history']                    // recipe_plan_history rows; feeds Auto Plan
```
